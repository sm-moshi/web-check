...
// Read and register each API function as Express routes
const apiFiles = fs
...
const renderPlaceholderPage = async (res, msgId, logs) => {
	const errorMessages = {
		notCompiled:
			"Looks like the GUI app has not yet been compiled.<br />Run <code>yarn build</code> to continue, then restart the server.",
		notCompiledSsrHandler:
			"Server-side rendering failed to initiate, as SSR handler not found.<br />This can be fixed by running <code>yarn build</code>, then restarting the server.<br />",
		disabledGui: `Web-Check API is up and running!<br />Access the endpoints at <a href="${API_DIR}"><code>${API_DIR}</code></a>`,
	};
