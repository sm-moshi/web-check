...
const featuresHandler = async (url) => {
	...
	const buildWappalyzerFallback = async (reason) => {
		...
		const technologies = Array.isArray(fallback.technologies)
	...
	if (!apiKey) {
		return buildWappalyzerFallback(
			"BuiltWith API key not configured (BUILT_WITH_API_KEY).",
		);
	}

	const apiUrl = `https://api.builtwith.com/free1/api.json?KEY=${apiKey}&LOOKUP=${encodeURIComponent(
		url,
	)}`;
	...
	try {
		...
		if (message.includes("status code: 429")) {
			return buildWappalyzerFallback(
				"BuiltWith rate limit exceeded (HTTP 429). Try again later.",
			);
		}
		if (
			...
			return buildWappalyzerFallback(
				"BuiltWith API plan does not permit this lookup.",
			);
		}
