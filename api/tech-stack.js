...
const normalizeBuiltWith = (response) => {
  ...
  return {
    technologies: technologies.map((tech) => {
      const categories = Array.isArray(tech.Categories) && tech.Categories.length
        ? tech.Categories
        : (tech.Tag ? [tech.Tag] : []);
      return {
        name: tech.Name,
...
const normalizeWappalyzergo = (response) => {
  const technologies = Array.isArray(response?.technologies) ? response.technologies : [];
  return {
    technologies: technologies.map((tech) => ({
      name: tech.name,
      version: tech.version || '',
...
const techStackHandler = async (url) => {
  ...
  try {
    ...
    throw new Error('Tech stack API key not configured. Set BUILT_WITH_API_KEY or WAPPALYZERGO_URL.');
  } catch (error) {
    const fallback = await fetchWappalyzergo(url);
    if (fallback) {
      return normalizeWappalyzergo(fallback);
