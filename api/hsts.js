...
const hstsHandler = async (url, _event, _context) => {
		...
		const req = https.request(url, (res) => {
			...
			if (!hstsHeader) {
				...
				if (!maxAgeMatch || parseInt(maxAgeMatch[1], 10) < 10886400) {
					...
					resolve(
						hstsIncompatible(`HSTS header does not include all subdomains.`),
					);
				} else if (!preload) {
					resolve(
						hstsIncompatible(
							`HSTS header does not contain the preload directive.`,
						),
					);
				} else {
					resolve(
