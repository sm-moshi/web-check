...
const firewallHandler = async (url) => {
	...
	try {
		...
		if (headers["x-powered-by"]?.includes("AWS Lambda")) {
		...
		if (headers.server?.includes("BarracudaWAF")) {
		...
		if (headers.server?.includes("FortiWeb")) {
			return hasWaf("Fortinet FortiWeb WAF");
		}

		if (headers.server?.includes("Imperva")) {
			return hasWaf("Imperva SecureSphere WAF");
		}

		if (headers["x-protected-by"]?.includes("Sqreen")) {
		...
		if (headers["set-cookie"]?.includes("_citrix_ns_id")) {
		...
		if (headers.server?.includes("Yundun")) {
		...
		if (headers.server?.includes("Safe3WAF")) {
			return hasWaf("Safe3 Web Application Firewall");
		}

		if (headers.server?.includes("NAXSI")) {
		...
		if (headers.server?.includes("QRATOR")) {
