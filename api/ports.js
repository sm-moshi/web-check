...
const DEFAULT_PORTS_TO_CHECK = [
	20, 21, 22, 23, 25, 53, 80, 67, 68, 69, 110, 119, 123, 143, 156, 161, 162,
	179, 194, 389, 443, 587, 993, 995, 3000, 3306, 3389, 5060, 5900, 8000, 8080,
	8888,
];
/*
...
const portsHandler = async (url, _event, _context) => {
	let host;
	try {
	...
	for (const promise of promises) {
		...
		if (result.status === "timeout") {
			...
			if (result.timeout) {
				...
				const portsNotChecked = PORTS.filter(
					(port) => !checkedPorts.includes(port),
				);
...
const errorResponse = (message, _statusCode = 444) => {
