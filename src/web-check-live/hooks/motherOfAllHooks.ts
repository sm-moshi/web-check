	...
	const doTheFetch = async () => {
		if (keys.disableEverything) {
			updateLoadingJobs(
				jobId,
				"skipped",
				"Web-Check is temporarily disabled. Please try again later.",
				reset,
			);
			return;
		}
		try {
			...
			if (!res) {
				...
				if (errorResult.error.includes("timed-out")) {
				...
				const errorMessage =
					`${errorResult.errorType}\n${errorResult.errorMessage}\n\n` +
					`If you're running your own instance, increase server timeouts or resources to handle this operation.`;
				updateLoadingJobs(jobId, "error", errorMessage, reset);
			} else if (errorResult && errorResult.skipped) {
