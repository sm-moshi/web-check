...
const FancyBackground = (): JSX.Element => {
	...
	const maxBy = (array: any) => {
		...
		for (const value of array) {
			...
			if (
				current != null &&
				(computed === undefined ? current : current > computed)
			) {
				computed = current;
				result = value;
			}
	...
	App.evolve = function () {
		...
		if (elemDead && elemDead.length > 0)
			elemDead[0].textContent = this.deathCount;

		const elemAlive = document.getElementsByClassName("alive");
		if (elemAlive && elemAlive.length > 0)
			elemAlive[0].textContent = this.particles.length;
		...
		if (elemDrawn && elemDrawn.length > 0)
			elemDrawn[0].textContent = this.drawnInLastFrame;
	};
	App.birth = function () {
		var x, y;
