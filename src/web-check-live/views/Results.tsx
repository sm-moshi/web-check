...
import {
	...
	type AddressType,
} from "web-check-live/utils/address-type-checker";
...
const StatusCard = (props: {
	...
	return (
		<Card heading={title} actionButtons={actionButtons}>
			<p>
				{statusLabel}.{" "}
				{message ? "Details below." : "No additional details provided."}
			</p>
		...
		fetchRequest: async () => {
			if (!ipAddress) {
				return Promise.resolve({
					skipped: "Shodan lookup requires an IP address.",
				});
			}
			...
			fetch(`${api}/features?url=${address}`)
				...
				.then((res) => {
					if (res.Errors && res.Errors.length > 0) {
						return {
							error: `No data returned, because ${res.Errors[0].Message || "API lookup failed"}`,
						};
					}
					return res;
				}),
				...
				>
					{resultCardData.map(
						(
							...
							const showStatus =
								!hasResult && jobState && jobState !== "loading";
