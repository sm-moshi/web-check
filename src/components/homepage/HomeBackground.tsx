...
const generateMeteor = (id: number, gridSizeX: number, gridSizeY: number) => {
	...
	const travelDistance =
		distanceBase + Math.floor(Math.random() * distanceVariance);
...
const WebCheckHomeBackground = () => {
	const [gridSizeX, setGridSizeX] = useState(
		Math.floor(window.innerWidth / dotSpacing),
	);
	const [gridSizeY, setGridSizeY] = useState(
		Math.floor(window.innerHeight / dotSpacing),
	);
	const [meteors, setMeteors] = useState(() =>
		generateInitialMeteors(gridSizeX, gridSizeY),
	);
					...
					if (meteor.animationStage === "traveling") {
						...
						setTimeout(
							() => {
								setMeteors((current) =>
									current.map((m) =>
										m.id === id ? generateMeteor(id, gridSizeX, gridSizeY) : m,
									),
								);
							},
